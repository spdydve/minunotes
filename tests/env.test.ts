...
import {
  ...
  getStageUrls,
  parseAllowedOrigins,
} from "../src/api/lib/env";

describe("parseAllowedOrigins", () => {
  it("parses and deduplicates configured origins", () => {
    expect(
      parseAllowedOrigins("https://app.example.com, https://app.example.com"),
    ).toEqual(["https://app.example.com", defaults.localFrontendUrl]);
  });

  it("falls back to the provided frontend url", () => {
    expect(parseAllowedOrigins(undefined, "https://notes.example.com")).toEqual(
      ["https://notes.example.com", defaults.localFrontendUrl],
    );
  });
  ...
  it("returns normalized runtime config", () => {
    expect(
      getApiRuntimeConfig({
        ...
        API_URL: "https://api.notes.example.com/",
        API_ALLOWED_ORIGINS:
          "https://notes.example.com, https://admin.example.com",
