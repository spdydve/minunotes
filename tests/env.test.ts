...
describe('parseAllowedOrigins', () => {
  ...
  it('normalizes origins by removing paths', () => {
    expect(parseAllowedOrigins('https://notes.example.com/internal/auth', 'https://notes.example.com')).toEqual([
      'https://notes.example.com',
      defaults.localFrontendUrl,
    ]);
  });
...
describe('getApiRuntimeConfig', () => {
  it('returns normalized runtime config', () => {
    expect(
      ...
      apiUrl: 'https://api.notes.example.com',
      betterAuthUrl: 'https://api.notes.example.com/internal/auth',
      allowedOrigins: ['https://notes.example.com', 'https://admin.example.com', defaults.localFrontendUrl],
