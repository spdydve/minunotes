...
describe("FilesystemObjectStorage", () => {
  it("stores, reads, and deletes objects", async () => {
    ...
    await expect(storage.getObject({ key: "users/user_1/notes/note_1/attachments/att_1-test.png" })).resolves.toBeNull();
  });
...
describe("attachment storage interface", () => {
  it("allows adapters to expose signed upload URLs", async () => {
    ...
    await expect(storage.createSignedUploadUrl?.({ key: "image.png", contentType: "image/png", expiresInSeconds: 900 })).resolves.toBe("https://uploads.example.com/image.png");
