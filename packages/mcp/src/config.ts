...
export function createClient(env: NodeJS.ProcessEnv = process.env): NotesMcpClient {
  ...
  return {
    ...
    notes: {
      ...
      create: (folderId, input) => request("/harness/notes", { method: "POST", body: JSON.stringify({ folderId, title: input.title, content: input.content }) }),
      edit: (noteId, edits, baseHash) => request(`/harness/notes/${encodeURIComponent(noteId)}/edit`, { method: "POST", body: JSON.stringify({ edits, baseHash }) }),
      searchLines: (input) => request(`/harness/notes/search-lines${toQueryString({ q: input.query, folderId: input.folderId, context: input.context, limit: input.limit, caseSensitive: input.caseSensitive })}`),
      lines: (noteId, input) => request(`/harness/notes/${encodeURIComponent(noteId)}/lines${toQueryString(input)}`),
      searchNoteLines: (noteId, input) => request(`/harness/notes/${encodeURIComponent(noteId)}/search-lines${toQueryString({ q: input.query, context: input.context, limit: input.limit, caseSensitive: input.caseSensitive })}`),
      section: (noteId, sectionId) => request(`/harness/notes/${encodeURIComponent(noteId)}/sections/${encodeURIComponent(sectionId)}`),
    },
  };
}
