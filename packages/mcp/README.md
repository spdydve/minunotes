...
Review comments are available only on active Markdown notes. Every comment operation requires explicit **Review comments** permission plus folder read access; note edit permission and API editability are not required. Existing and new credentials have Review comments disabled until explicitly granted. Message edits/deletes are author-only, and anchor creation or remapping requires the current note content hash.
