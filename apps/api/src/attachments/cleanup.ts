...
export async function cleanupUnreferencedAttachments(input: { cutoff?: Date; limit?: number } = {}) {
  ...
  const candidates = await db.select().from(attachments)
    .where(and(isNotNull(attachments.unreferencedAt), isNull(attachments.deletedAt), lt(attachments.unreferencedAt, cutoff)))
    .limit(limit);
  ...
  for (const attachment of candidates) {
    ...
    await db.update(attachments)
      ...
      .where(and(eq(attachments.id, attachment.id), isNull(attachments.deletedAt), lt(attachments.unreferencedAt, cutoff)));
    deleted += 1;
  }
