...
import { applyNoteTheme, getStoredTheme, noteThemes, type NoteThemeId } from "../lib/themes";
...
export function ThemeSelect() {
  ...
  return <label className="block px-3 py-2 text-xs text-[var(--notes-muted)]">
    ...
    >
      {noteThemes.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}
    </select>
  </label>;
}
