...
function NoteActivityView() {
  ...
  if (note.error instanceof ApiError && note.error.status === 404) return <section className="grid min-h-[60vh] place-items-center"><div className="w-full max-w-lg rounded-lg border border-dashed p-8 text-center"><h2 className="text-xl font-semibold">Note not found</h2><p className="mt-2 text-sm text-slate-500">This note does not exist or you do not have access to it.</p><Link className="mt-4 inline-block rounded-md border border-slate-300 px-3 py-2 text-sm hover:bg-slate-100 dark:border-slate-700 dark:hover:bg-slate-900" to="/">Back to notes</Link></div></section>;
  ...
  return <section className="mx-auto w-full max-w-4xl space-y-4">
    <div className="flex flex-wrap items-center justify-between gap-3">
      ...
      <Link className="rounded-md border border-slate-300 px-3 py-2 text-sm hover:bg-slate-100 dark:border-slate-700 dark:hover:bg-slate-900" to="/notes/$noteId" params={{ noteId }}>Back to note</Link>
    </div>
...
export const noteActivityRoute = createRoute({ getParentRoute: () => rootRoute, path: "/notes/$noteId/activity", component: NoteActivityView });
