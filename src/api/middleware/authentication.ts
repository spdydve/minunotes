...
export const authenticationMiddleware = createMiddleware(async (c, next) => {
  ...
  setAuthState(c, {
    ...
    authContext: session?.user && session.session
...
function logIntegrationAuth(c: Parameters<Parameters<typeof createMiddleware>[0]>[0], authContext: AuthContext) {
  ...
  console.info("[INTEGRATION AUTH]", {
    ...
    authId: authContext.type === "apiKey" ? authContext.apiKeyId : authContext.type === "oauth" ? authContext.authorizationId : null,
  });
}
...
export const mcpOAuthAuthenticationMiddleware = createMiddleware(async (c, next) => {
  ...
  if (!bearer) return c.json({ error: "Unauthorized" }, 401, authChallenge ? { "WWW-Authenticate": authChallenge } : undefined);

  const row = await authenticateOAuthBearer(bearer);
  if (!row) return c.json({ error: "Invalid bearer token" }, 401, authChallenge ? { "WWW-Authenticate": authChallenge } : undefined);
