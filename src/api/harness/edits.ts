...
export type EditResult = { ok: true; markdown: string } | { ok: false; error: string };
...
export function applyDocumentEdits(markdown: string, edits: DocumentEdit[]): EditResult {
  ...
  for (const edit of edits) {
    ...
    if (!Number.isInteger(edit.from) || !Number.isInteger(edit.to))
      return { ok: false, error: 'replace_range bounds must be integers' };
    if (edit.from < 0 || edit.to < edit.from || edit.to > markdown.length)
      return { ok: false, error: 'replace_range bounds are invalid' };
