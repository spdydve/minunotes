...
export function NoteVersionsDialog({
  ...
  const versionsQuery = useQuery({
    ...
    queryFn: () => api.noteVersions(note.id),
    enabled: open,
  });
  ...
  const restore = useMutation({
    mutationFn: async (versionId: string) => {
      if (!window.confirm('Restore this version? The current note state will be saved first.'))
          ...
          <div className="max-h-[65vh] overflow-y-auto rounded-lg border border-[var(--notes-border)]">
            ...
            {versions.map((version) => (
              <button
                key={version.id}
                type="button"
                className={`block w-full border-b border-[var(--notes-border)] px-3 py-2 text-left text-sm last:border-b-0 hover:bg-[var(--notes-hover)] ${selectedSummary?.id === version.id ? 'bg-[var(--notes-hover)]' : ''}`}
              ...
              <div className="flex items-start justify-between gap-3 border-b border-[var(--notes-border)] p-3">
                <div className="min-w-0">
                  <p className="truncate text-sm font-semibold">
                    {selectedVersion.data?.version.title ?? selectedSummary.title}
                  </p>
                  <p className="notes-muted mt-1 text-xs">
                    {reasonLabel(selectedSummary.reason)} · {formatDate(selectedSummary.createdAt)} ·{' '}
                    {actorLabel(selectedSummary)}
                  </p>
                </div>
                <Button
                  className="px-2 py-1 text-xs"
                  disabled={restore.isPending || selectedVersion.isLoading}
                  onClick={() => selectedSummary && restore.mutate(selectedSummary.id)}
                >
                  Restore
                </Button>
              </div>
            ...
            <div className="max-h-[58vh] overflow-y-auto p-4">
              ...
              {selectedVersion.data?.version ? (
                <pre className="whitespace-pre-wrap break-words font-mono text-sm leading-6 text-[var(--notes-text)]">
                  {selectedVersion.data.version.content || '(Empty note)'}
                </pre>
              ) : null}
            </div>
          </div>
