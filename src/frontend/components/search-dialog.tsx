  ...
  return <>
    ...
    {open && <div className="notes-overlay fixed inset-0 z-50 grid place-items-center p-4">
      <div className="notes-card max-h-[calc(100dvh-2rem)] w-full max-w-xl overflow-y-auto rounded-lg p-4 shadow-sm sm:p-5">
        ...
        <input autoFocus className="notes-input mt-4 w-full rounded-md px-3 py-2" placeholder="Search notes or folders..." value={query} onChange={(e) => setQuery(e.target.value)} />
        <div className="mt-4 max-h-[min(28rem,calc(100dvh-12rem))] space-y-4 overflow-auto">
          {folderMatches.length > 0 ? <section>
            <p className="mb-2 text-xs font-medium uppercase tracking-wide text-[var(--notes-muted)]">Folders</p>
            <div className="space-y-2">
              {folderMatches.map((folder) => <button key={folder.id} className="block w-full rounded-md border border-[var(--notes-border)] p-3 text-left transition-colors hover:bg-[var(--notes-hover)]" onClick={() => { setOpen(false); nav({ to: "/folders/$folderId", params: { folderId: folder.id } }); }}>
          ...
          <section>
            {folderMatches.length > 0 ? <p className="mb-2 text-xs font-medium uppercase tracking-wide text-[var(--notes-muted)]">Notes</p> : null}
            {isFetching && <p className="notes-muted text-sm">Searching...</p>}
            {!isFetching && trimmed && !data?.notes.length && !folderMatches.length && <p className="notes-muted text-sm">No notes or folders found.</p>}
            {!isFetching && trimmed && !data?.notes.length && folderMatches.length > 0 && <p className="notes-muted text-sm">No matching notes found.</p>}
            <div className="space-y-2">
