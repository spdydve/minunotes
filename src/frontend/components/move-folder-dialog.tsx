    ...
    ? destinationDisabledReason(
        {
          ...destinationFolder,
          children: [],
          depth: getFolderDepth(destinationFolder, folders),
          effectivePrivate: isEffectivelyPrivate(destinationFolder, folders),
          effectiveAgentReadOnly: isEffectivelyAgentReadOnly(destinationFolder, folders),
        },
        folder,
        folders,
        height
      ...
      <div className="notes-card max-h-[calc(100dvh-2rem)] w-full max-w-md overflow-y-auto rounded-lg p-4 shadow-sm sm:p-5">
        ...
        <div className="mt-4 flex justify-end gap-2">
          ...
          <Button
            type="button"
            disabled={
              move.isPending || destinationFolderId === currentParentId || Boolean(destinationDisabledReasonText)
            }
            onClick={() => {
              setError(null);
              move.mutate();
