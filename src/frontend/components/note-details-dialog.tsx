...
export function NoteDetailsDialog({ note, open, onOpenChange, onNoteUpdated }: { note: Note; open: boolean; onOpenChange: (open: boolean) => void; onNoteUpdated?: (response: { note: Note; contentHash: string }) => void }) {
  ...
  const { data: tagsData, isLoading } = useQuery({ queryKey: ["note-tags", note.id], queryFn: () => api.noteTags(note.id), enabled: open });

  const folderTitle = useMemo(() => foldersData?.folders.find((folder) => folder.id === note.folderId)?.title ?? note.folderId, [foldersData, note.folderId]);
  ...
  const updatedBy = note.updatedByActorType === "agent"
  ...
  return createPortal(<div className="notes-overlay fixed inset-0 z-[100] grid place-items-center p-4">
    <div className="w-full max-w-xl overflow-hidden rounded-2xl border border-[var(--notes-border)] bg-[var(--notes-panel)] text-[var(--notes-text)] shadow-xl">
      <div className="flex items-center justify-between gap-3 border-b border-[var(--notes-border)] px-5 py-4">
        ...
        <button type="button" className="rounded-md p-1.5 text-[var(--notes-muted)] hover:bg-[var(--notes-hover)] hover:text-[var(--notes-text)]" onClick={() => onOpenChange(false)} aria-label="Close details dialog">
          <X className="h-4 w-4" />
        </button>
      </div>
      ...
      <div className="space-y-5 px-5 py-5">
        <dl className="grid gap-3 text-sm sm:grid-cols-[140px_1fr]">
          <dt className="notes-muted self-center">Name</dt><dd><input className="w-full rounded-md border border-[var(--notes-border)] bg-[var(--notes-bg)] px-2 py-1.5 outline-none focus:border-[var(--notes-blue)]" value={title} onChange={(event) => setTitle(event.target.value)} /></dd>
          <dt className="notes-muted">Folder</dt><dd>{folderTitle}</dd>
          <dt className="notes-muted">Type</dt><dd>{note.type === "template" ? "Template" : "Note"}</dd>
          <dt className="notes-muted self-center">Created</dt><dd><input className="w-full rounded-md border border-[var(--notes-border)] bg-[var(--notes-bg)] px-2 py-1.5 outline-none focus:border-[var(--notes-blue)]" type="datetime-local" value={createdAt} onChange={(event) => setCreatedAt(event.target.value)} /></dd>
          <dt className="notes-muted">Updated</dt><dd>{formatDate(note.updatedAt)}</dd>
          <dt className="notes-muted">Updated by</dt><dd>{updatedBy}</dd>
          <dt className="notes-muted self-center">API editable</dt><dd><label className="inline-flex items-center gap-2"><input type="checkbox" checked={isApiEditable} onChange={(event) => setIsApiEditable(event.target.checked)} /> <span>{isApiEditable ? "Enabled" : "Disabled"}</span></label></dd>
        </dl>

        <section className="space-y-2 border-t border-[var(--notes-border)] pt-5">
          <label className="block text-sm font-medium" htmlFor="note-tags-input">Tags</label>
          <div className="relative flex gap-2">
            ...
            />
            {tagDraft.trim() ? <button type="button" className="rounded-lg border border-[var(--notes-border)] px-3 py-2 text-sm font-medium hover:bg-[var(--notes-hover)]" onClick={() => addTag(tagDraft)}>Add</button> : null}
            {tagInputFocused && suggestions.length > 0 ? <div className="absolute left-0 top-full z-10 mt-1 w-full max-w-sm overflow-hidden rounded-lg border border-[var(--notes-border)] bg-[var(--notes-panel)] p-1 text-sm shadow-lg">
              {suggestions.map((tag) => <button key={tag.id} type="button" className="block w-full rounded-md px-3 py-2 text-left hover:bg-[var(--notes-hover)]" onMouseDown={(event) => { event.preventDefault(); addTag(tag.name); setTagInputFocused(false); }}>#{tag.name}</button>)}
            </div> : null}
          </div>
          <p className="notes-muted text-xs">Tags are lowercase words and may use dashes, e.g. plan or plan-type.</p>
          {tagNames.length > 0 ? <div className="flex flex-wrap gap-2 pt-1">
            {tagNames.map((tag) => <span key={tag} className="inline-flex items-center gap-1 rounded-full border border-[var(--notes-border)] bg-[var(--notes-bg)] px-2 py-1 text-xs font-medium">
              #{tag}
              <button
                type="button"
        ...
        <div className="flex justify-end gap-2 border-t border-[var(--notes-border)] pt-4">
          <button type="button" className="rounded-lg border border-[var(--notes-border)] px-3 py-2 text-sm font-medium hover:bg-[var(--notes-hover)]" onClick={() => onOpenChange(false)}>Close</button>
          <button type="button" className="inline-flex items-center gap-2 rounded-lg bg-[var(--notes-blue)] px-3 py-2 text-sm font-semibold text-white hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60" disabled={saveDetails.isPending} onClick={() => saveDetails.mutate()}>
