...
export function ActionMenuIconButton({
  ...
  className,
  ...props
}: ButtonHTMLAttributes<HTMLButtonElement> & { icon?: 'more' | 'settings' }) {
...
export function ActionMenuButton({
  ...
  className,
  ...props
}: ButtonHTMLAttributes<HTMLButtonElement> & { children: ReactNode; destructive?: boolean }) {
  return (
    <button type="button" className={actionMenuItemClass(destructive, className)} {...props}>
      {children}
    </button>
  );
}

export function ActionMenuItemLabel({
  children,
  destructive = false,
  className,
  ...props
}: HTMLAttributes<HTMLSpanElement> & { children: ReactNode; destructive?: boolean }) {
  return (
    <span className={actionMenuItemClass(destructive, className)} {...props}>
      {children}
    </span>
  );
}
