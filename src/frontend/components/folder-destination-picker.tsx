...
export function FolderDestinationPicker({
  ...
  return <div className="space-y-3">
    ...
    <div className="relative">
      ...
      <input className="notes-input w-full rounded-md py-2 pl-9 pr-3" placeholder="Search folders" value={query} onChange={(event) => setQuery(event.target.value)} />
    </div>

    <div className="max-h-72 space-y-1 overflow-y-auto rounded-md border border-[var(--notes-border)] p-1">
      {visibleRows.length ? visibleRows.map((folder) => {
        ...
        return <button
          ...
          onClick={() => { setQuery(""); onCurrentFolderIdChange(folder.id); }}
        >
