        ...
        <table className="w-full border-collapse text-sm">
          <thead className="bg-[var(--notes-table-header-bg)]">
            {table.getHeaderGroups().map((hg) => (
              <tr key={hg.id}>
                {hg.headers.map((h) => (
                  <th className="border-b border-[var(--notes-border)] px-4 py-2.5 text-left text-xs font-medium uppercase tracking-wide text-[var(--notes-muted)] first:pl-5 last:pr-5" key={h.id}>
            ...
            {table.getRowModel().rows.map((row) => (
              <tr key={row.id} className="transition-colors hover:bg-[var(--notes-table-row-hover)]">
                {row.getVisibleCells().map((cell) => (
                  <td className="border-b border-[var(--notes-table-row-border)] px-4 py-3 align-middle first:pl-5 last:pr-5 last:text-right" key={cell.id}>
