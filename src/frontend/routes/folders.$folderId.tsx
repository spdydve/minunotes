      ...
      {items.map((item) => item.kind === "folder" ? <div key={`folder-${item.folder.id}`} className="rounded-lg border border-[var(--notes-border)] bg-[var(--notes-panel)] p-4">
        ...
        <div className="flex items-start justify-between gap-3">
          <Link to="/notes/$noteId" params={{ noteId: item.note.id }} className="flex min-w-0 flex-1 items-start gap-3">
            <span className="rounded-md border border-[var(--notes-border)] bg-[var(--notes-panel-muted)] p-2 text-[var(--notes-muted)]">{item.note.documentType.startsWith("canvas.") ? <Shapes className="h-4 w-4" /> : <FileText className="h-4 w-4" />}</span>
            <span className="min-w-0">
              <span className="flex min-w-0 items-center gap-2 font-medium hover:text-[var(--notes-blue)]">
          ...
          {items.map((item) => <tr key={item.kind === "folder" ? `folder-${item.folder.id}` : `note-${item.note.id}`} className="transition-colors hover:bg-[var(--notes-table-row-hover)]">
            <td className="border-b border-[var(--notes-table-row-border)] px-5 py-3 align-middle">
              {item.kind === "folder" ? <Link to="/folders/$folderId" params={{ folderId: item.folder.id }} className="flex min-w-0 items-center gap-3 font-medium hover:text-[var(--notes-blue)]">
                ...
                {item.note.documentType.startsWith("canvas.") ? <Shapes className="h-4 w-4 shrink-0 text-[var(--notes-muted)]" /> : <FileText className="h-4 w-4 shrink-0 text-[var(--notes-muted)]" />}
                <span className="truncate">{item.note.title}</span>
              </Link>}
            </td>
    ...
    <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex flex-wrap items-center justify-end gap-2">
        <div className="inline-flex overflow-hidden rounded-md border border-[var(--notes-button-secondary-border)]">
          <button type="button" className="border-r border-[var(--notes-button-secondary-border)] bg-[var(--notes-button-secondary-bg)] px-3 py-2 text-sm text-[var(--notes-button-secondary-text)] transition-colors hover:bg-[var(--notes-button-secondary-hover)] disabled:cursor-not-allowed disabled:opacity-50" disabled={create.isPending} onClick={createMarkdownNote}>New Note</button>
          <Popover open={newMenuOpen} onOpenChange={setNewMenuOpen}>
            <PopoverTrigger asChild>
              <button type="button" className="flex items-center bg-[var(--notes-button-secondary-bg)] px-2 text-sm text-[var(--notes-button-secondary-text)] transition-colors hover:bg-[var(--notes-button-secondary-hover)] disabled:cursor-not-allowed disabled:opacity-50" disabled={create.isPending} aria-label="Open new options">
                <ChevronDown className="h-4 w-4" />
              </button>
            </PopoverTrigger>
            <PopoverContent align="end" className="w-48 p-1">
              <button type="button" className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-sm hover:bg-[var(--notes-hover)]" onClick={openTemplatePicker}>
