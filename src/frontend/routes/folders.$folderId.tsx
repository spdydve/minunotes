...
function FolderView() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ["notes", folderId], queryFn: () => api.notes(folderId) });
  ...
  return <section><div className="mb-4 flex items-center justify-between"><h2 className="text-xl font-semibold">Folder notes</h2><div className="flex gap-2"><Button onClick={() => create.mutate()}>New note</Button><DeleteConfirmDialog label="folder" warning="All notes in this folder will be permanently lost." onConfirm={() => remove.mutate()} /></div></div>{isLoading ? <p>Loading...</p> : data?.notes.length ? <NotesTable notes={data.notes} /> : <p className="rounded-lg border border-dashed p-8 text-sm text-slate-500">No notes yet.</p>}</section>;
}
