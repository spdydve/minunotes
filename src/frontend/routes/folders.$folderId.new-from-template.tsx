...
import { ApiError, api, type NoteListItem } from '../lib/api';
...
function NewFromTemplateView() {
  ...
  const selectedQuery = useQuery({
    ...
    queryFn: () => api.note(selectedId ?? ''),
    enabled: Boolean(selectedId),
  });
  const selected = selectedQuery.data?.note ?? null;
  ...
  return (
    <section className="mx-auto w-full max-w-5xl">
      <div className="mb-6 flex flex-col gap-2">
        ...
        <h2 className="font-semibold text-xl">New note from template</h2>
      </div>
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(20rem,0.8fr)]">
          <div className="space-y-3">
            <label
              ...
              className="block font-medium text-[var(--notes-muted)] text-xs uppercase tracking-wide"
            >
              Note title
            </label>
            <input
            ...
            />
            <p className="pt-2 font-medium text-[var(--notes-muted)] text-xs uppercase tracking-wide">
              Assigned templates
            </p>
            {templates.data.templates.map((template: NoteListItem) => (
              <button
                type="button"
                key={template.id}
              ...
              >
                ...
                <span className="notes-muted mt-1 block text-xs">
                  {template.id === selectedSummary?.id && selectedQuery.isFetching ? 'Loading preview…' : 'Template'}
                </span>
              </button>
            ))}
          </div>
          <div className="rounded-lg border border-[var(--notes-border)] bg-[var(--notes-panel-muted)] p-3 text-sm">
            <p className="mb-2 font-medium text-[var(--notes-muted)] text-xs uppercase tracking-wide">Preview</p>
            <pre className="max-h-[30rem] overflow-auto whitespace-pre-wrap font-mono text-[var(--notes-text)] text-xs">
              {selected
          ...
          <div className="flex gap-2 lg:col-span-2">
            <Button
              ...
              disabled={!selected || selectedQuery.isFetching || create.isPending}
            >
              Create note
            </Button>
